...
def check_available_ncku_tainan(url :str) -> bool:
    ...
    for i, date in enumerate(optionValues):
        ...
        appointments = list(map(lambda x: x.text, filter(filter_appointments_ncku_tainan, table.find_all("td", {"class": "p-0"}))))
        ...
        if i != len(optionValues) - 1:
            ...
            post_data["__VIEWSTATE"] = soup.find("input", {"id": "__VIEWSTATE"}).get("value")
            post_data["__VIEWSTATEGENERATOR"] = soup.find("input", {"id": "__VIEWSTATEGENERATOR"}).get("value")
            post_data["__EVENTVALIDATION"] = soup.find("input", {"id": "__EVENTVALIDATION"}).get("value") 
...
def filter_appointments_ncku_tainan(element):
    sub_strs = ["預約", "轉掛", "停診"]
